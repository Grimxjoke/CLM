...
contract StrategyPassiveManagerUniswap is StratFeeManagerInitializable, IStrategyConcLiq, IStrategyUniswapV3 {
    ...
    // Events
    event Deposit(uint256 bal0, uint256 bal1);
    event Withdraw(uint256 bal0, uint256 bal1);
    ...
    function rebalance() external onlyCalmPeriods onlyRebalancers {
        _claimEarnings();
        _removeLiquidity();
        _setTicks();
        _addLiquidity();
    ...
    function _chargeFees(address _callFeeRecipient, uint256 _amount0, uint256 _amount1) private returns (uint256 _amountLeft0, uint256 _amountLeft1){
        ...
        if (_amount0 > 0) {
            ...
            uint256 out0;
            if (lpToken0 != native) out0 = UniV3Utils.swap(unirouter, lpToken0ToNativePath, amountToSwap0);
        ...
        if (_amount1 > 0) {
            ...
            uint256 out1;
            if (lpToken1 != native) out1 = UniV3Utils.swap(unirouter, lpToken1ToNativePath, amountToSwap1);
